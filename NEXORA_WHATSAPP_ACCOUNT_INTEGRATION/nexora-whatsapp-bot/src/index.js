...
async function start(){
  ...
  const sock=makeWASocket({auth:authState,version,logger,printQRInTerminal:false,markOnlineOnConnect:false,syncFullHistory:false,generateHighQualityLinkPreview:false});
  sock.ev.on('creds.update',saveCreds);
  sock.ev.on('connection.update',async({connection,lastDisconnect,qr})=>{
    if(qr){console.log('\nScan this QR code with the WhatsApp account that will run NEXORA Community Bot:\n');qrcode.generate(qr,{small:true});}
    if(connection==='open'){console.log('\n✅ NEXORA Community Bot connected.');console.log(`Group restriction: ${GROUP_JID||'ALL GROUPS WHERE BOT IS ADMIN'}`);console.log(`Policy: member links blocked; admins allowed; ${MAX_STRIKES} strikes = removal.`);console.log(`AI: ${AI_API_KEY?'enabled':'verified knowledge-base mode'}`);console.log('Commands: /nexora /rules /stats /announce /strikes /clearstrike\n');await listGroups(sock);}
    if(connection==='close'){const code=lastDisconnect?.error?.output?.statusCode;const shouldReconnect=code!==DisconnectReason.loggedOut;logger.warn({code,shouldReconnect},'WhatsApp connection closed');if(shouldReconnect)setTimeout(start,3000);else console.error('❌ WhatsApp session logged out. Delete auth/ and link the account again.');}
  });
